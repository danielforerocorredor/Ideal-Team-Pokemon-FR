...
def grafo():
    ...
    nodesP = ["Isla Canela","Pueblo Paleta","Ciudad Verde"]
    ...
    print("Ruta mas corta usando el algoritmo de Dijkstra entre Isla Canela y Ciudad Verde: ",AD8)
    g=kanto.subgraph(AD8)

    pos1 = nx.get_node_attributes(kanto, 'pos')
    weight1=nx.get_edge_attributes(kanto,'weight')


    nx.draw_networkx(g,pos1,node_color = color_map,edge_color=colore)
    nx.draw_networkx_edge_labels(kanto,pos1,edge_labels = weight1)
    plt.show()
    fig.savefig("IslaCanela_CuidadVerde.png")
